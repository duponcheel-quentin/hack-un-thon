...
function getSheet($id) {
  $db = getDataBase();
  $query = $db->prepare("SELECT * FROM sheets WHERE sheet_id = ?");
  $query->execute([$id]);
...
function updateSheet($sheet, $id) {
  $db = getDataBase();
  $query = $db->prepare("UPDATE sheets SET organization = :organization, entitled = :entitled, start = :start, end = :end, monday_morning = :monday_morning, monday_afternoon = :monday_afternoon, tuesday_morning = :tuesday_morning, tuesday_afternoon = :tuesday_afternoon, wednesday_morning = :wednesday_morning, wednesday_afternoon = :wednesday_afternoon, thursday_morning = :thursday_morning, thursday_afternoon = :thursday_afternoon, friday_morning = :friday_morning, friday_afternoon = :friday_afternoon WHERE sheet_id = :sheet_id)");
  $result = $query->execute([
    "organization" => $sheet["organization"] ,
    "entitled" => $sheet["entitled"],
    "start" => $sheet["start"],
...
function deleteSheet($id) {
  $db = getDataBase();
  $query = $db->prepare("DELETE FROM sheets WHERE sheet_id = ?");
...
function getSheetByUser($id){
    $db = getDataBase();
    $reponses = $db->prepare("SELECT s.entitled, s.start, s.end FROM sheets AS s
